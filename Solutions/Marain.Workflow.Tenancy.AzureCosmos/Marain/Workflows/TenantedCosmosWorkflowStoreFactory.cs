...
namespace Marain.Workflows
{
    ...
    public class TenantedCosmosWorkflowStoreFactory : ITenantedWorkflowStoreFactory
    {
        ...
        {
            Container container = await Retriable.RetryAsync(() => this.containerFactory.GetContainerForTenantAsync(tenant, this.containerDefinition), CancellationToken.None, new Linear(TimeSpan.FromSeconds(5), 5), new RetryOnCosmosRequestRateExceededPolicy()).ConfigureAwait(false);
