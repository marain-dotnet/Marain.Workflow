...
Scenario: Store a new workflow definition
	...
	When I store the workflow called 'workflow1-1' in the Azure storage workflow store
...
Scenario: Attempt to store a new workflow definition when a workflow with the same Id already exists
	...
	And I have stored the workflow called 'workflow2-1' in the Azure storage workflow store
	And I have a workflow definition with Id 'workflow2-1' called 'workflow2-2'
	When I store the workflow called 'workflow2-2' in the Azure storage workflow store
...
Scenario: Update a workflow definition
	...
	And I have stored the workflow called 'workflow5-1' in the Azure storage workflow store
	And I change the description of the workflow definition called 'workflow5-1' to 'A new description'
	When I store the workflow called 'workflow5-1' in the Azure storage workflow store
	Then the request is successful
...
Scenario: Update a workflow definition where the workflow etag does not match the most recent stored version
	...
	And I have stored the workflow called 'workflow6-1' in the Azure storage workflow store
	...
	When I store the workflow called 'workflow6-1' in the Azure storage workflow store
