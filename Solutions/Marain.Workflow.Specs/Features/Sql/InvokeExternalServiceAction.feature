...
Scenario: External action invoked
    ...
    And the workflow instance with Id 'id1' should contain context items
	| Key         | Value  |
	| include1    | value1 |
	| include2    | value2 |
	| dontinclude | value3 |
...
@externalServiceRequired
Scenario: External action returns a 200 status code and instance context updates
    Given I have created and persisted a workflow containing an external action with id 'external-action-workflow'
	...
	| dontinclude | value3 |
    And the external service will return a 200 status
    And the external service response will specify context items to upsert
    | Key      | Value  |
    | include1 | value4 |
    | added1   | value5 |
    And the external service response will specify context items to remove
    | Key      |
    | include2 |
	And the workflow trigger queue is ready to process new triggers
    ...
    And the request body ContextProperties key 'include2' has the value 'value2'
    And the request body ContextProperties has 2 values
	Then the workflow instance with Id 'id2' should have status 'Complete'
	And the workflow instance with Id 'id2' should be in the state called 'Done'
    And the workflow instance with Id 'id2' should contain context items
	| Key         | Value  |
	| include1    | value4 |
	| dontinclude | value3 |
	| added1      | value5 |
...
Scenario: External action returns a 500 status code
    Given I have created and persisted a workflow containing an external action with id 'external-action-workflow'
	And I have created and persisted a new instance with Id 'id3' of the workflow with Id 'external-action-workflow' and supplied the following context items
	| Key         | Value  |
	| include1    | value1 |
	| include2    | value2 |
	| dontinclude | value3 |
	...
	And the workflow trigger queue is ready to process new triggers
    When I send a trigger that will execute the action with a trigger id of 'id3'
	And I wait for all triggers to be processed
    Then the action endpoint should have been invoked
